package hu.ponte.hr;

import hu.ponte.hr.services.SignService;
import org.apache.commons.io.IOUtils;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.junit4.SpringRunner;

import java.io.FileInputStream;
import java.util.LinkedHashMap;
import java.util.Map;

import static org.junit.jupiter.api.Assertions.assertEquals;

/**
 * @author zoltan
 */
@RunWith(SpringRunner.class)
@SpringBootTest()
public class SignTest {

	@Autowired
	SignService signService;

	Map<String, String> files = new LinkedHashMap<String, String>() {
		{
			put("cat.jpg","");
			put("enhanced-buzz.jpg","");
			put("rnd.jpg","");
		}
	};

	@Test
	public void test_01() {
		files.forEach((fileName, encodedSign) -> {
			FileInputStream input = getFileInputStream(fileName);
			byte[] fileContent = getFileContent(input);
			String actualSign = signService.signSHA256RSA(fileContent);
			assertEquals(encodedSign, actualSign);
		});
	}

	private FileInputStream getFileInputStream(String fileName) {
		try {
			return new FileInputStream("src/test/resources/images/" + fileName);
		} catch (Exception e) {
			return null;
		}
	}

	private byte[] getFileContent(FileInputStream input) {
		try {
			return IOUtils.toByteArray(input);
		} catch (Exception e) {
			return null;
		}
	}
}
